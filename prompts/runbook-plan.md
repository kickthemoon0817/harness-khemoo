...
3. **Visual check by independent eyes (V5).** For every behaviour change (merge class B), spawn a
...
4. **Merge rule by class** (the issue states the class):
...
5. **Principle checks in every PR body**: P1 (the diff names no body role — grep the diff for
...
6. **Tick cost (P8, owner decision D4).** Correctness first: a change may slow the tick, but a PR
...
7. **Void mission-4 rules:** "PHYSICS FIRST" ordering, "LIVE VERDICT SIZE", the perf-drift and
...
8. **Plan documents are the operator's.** Never edit `PRINCIPLE.md`, `TODO.md`, `DECISIONS.md` or
